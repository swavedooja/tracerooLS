...
import { PackagingThreedViewComponent } from '../../../../shared/components/packaging-threed-view/packaging-threed-view.component';
...
export class ShippingLabelManagementComponent implements OnInit {
  ...
  async loadHierarchies() {
    try {
      const data = await this.labelService.getHierarchies();
  ...
  async loadLevels(hid: string) {
    try {
      this.levels = await this.labelService.getLevels(hid);
    } catch (e) {
      console.error(e);
    }
  }
  ...
  async createHierarchy() {
    ...
    try {
      const h = await this.labelService.createHierarchy({ name: `Shipping - ${baseH.name}` });
      await this.labelService.createLevel({
        hierarchy_id: h.id,
  ...
  async deleteLevel(id: string) {
    if (confirm('Delete this level?')) {
      await this.labelService.deleteLevel(id);
      this.loadLevels(this.selectedHierarchy.id);
    }
  }
  ...
  inferShape(name: string) {
    ...
    if (n.includes('container') || n.includes('export')) return 'Carton';
    return 'Box';
  }
  ...
  getThreeDLevels() {
    return this.levels.map(l => ({
      ...
      shapeType: this.inferShape(l.level_name) as any
