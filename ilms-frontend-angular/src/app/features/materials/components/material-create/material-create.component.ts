...
export class MaterialCreateComponent implements OnInit {
  ...
  async loadMaterial(code: string) {
    this.loading = true;
    try {
      const data = await this.materialService.getMaterial(code);
  ...
  async submit() {
    ...
    try {
      ...
      if (this.mode === 'create') {
        await this.materialService.createMaterial(payload);
        this.snackBar.open('Material created successfully!', 'Close', { duration: 3000 });
      } else {
        await this.materialService.updateMaterial(this.code!, payload);
