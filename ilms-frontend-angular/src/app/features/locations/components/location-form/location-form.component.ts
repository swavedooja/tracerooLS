...
export class LocationFormComponent implements OnInit, OnChanges {
  ...
  async loadMasters() {
    try {
      const [defs, locs] = await Promise.all([
        this.dashboardService.getMasterDefinitions(),
        this.locationService.getRoots() // Simplified for now, should ideally get all
  ...
  async loadData() {
    ...
    this.loading = true;
    try {
      const data = await this.locationService.getLocation(this.code!);
  ...
  async save() {
    ...
    try {
      if (this.code === 'new') {
        await this.locationService.createLocation(this.form.value);
        this.snackBar.open('Location created', 'Close', { duration: 3000 });
      } else {
        await this.locationService.updateLocation(this.code!, this.form.value);
  ...
  async handleDelete() {
    ...
    if (confirm('Are you sure you want to delete this location?')) {
      try {
        await this.locationService.removeLocation(this.code);
